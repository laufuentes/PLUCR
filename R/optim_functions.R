...
#' @export
SGD <- function(theta_current, psi, X, delta_Mu, delta_Nu, lambda, alpha=0.05, beta=0.05, centered=FALSE,
...
#' 
#' @export
SGD_X <- function(theta_current, psi_X, X, delta_Mu_X, delta_Nu_X, lambda, alpha=0.05, beta=0.05, centered=FALSE,
...
#' 
#' @export
FW_X <- function(X, delta_Mu, delta_Nu, lambda, alpha=0.05, beta=0.05, centered=FALSE, precision=0.05, verbose=TRUE) {
  K <- as.integer(1/precision)
  tol <- 1e-5
  d <- ncol(X)
