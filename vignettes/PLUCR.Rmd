...
```{r, eval=FALSE}
oracular_output <- oracular_approach_algorithm(X=X, A=A, Y=Y, Xi=Xi, folds=folds, Lambdas = Lambda_candidates,
...
eval_oracular <- oracular_process_results(theta=theta_oracular, X=X_test,  delta_Mu = delta_mu, delta_Nu = delta_nu, 
                                       scenario_mu = scenario_mu, scenario_nu = scenario_nu, 
                                       lambda = attr(theta_oracular, "lambda"), alpha = alpha.tolerance,
...
data <- tibble(
  ...
  constraint = c(eval_0$constraint, eval_naive$constraint, eval_final$constraint, eval_oracular$policy_value)
)

# Call the function
plot_metric_comparison(data, metrics = select(data, -"method") %>% colnames(), techniques = data$method)
```
